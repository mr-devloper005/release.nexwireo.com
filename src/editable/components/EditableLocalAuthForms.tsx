...
const inputClass = 'h-[3.25rem] rounded-xl border border-[#b9dce4] bg-white px-4 text-base font-bold text-black outline-none transition placeholder:text-black/35 focus:border-[#006d86]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-full bg-[#38c3cc] px-6 text-sm font-black text-black transition hover:bg-[#08263b] hover:text-white disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-xl border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
