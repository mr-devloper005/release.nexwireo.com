...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#dff6fb] text-[#07131c]">
        <section className="teal-geometry text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-white/75">The newsroom</p>
              <h1 className="mt-3 text-5xl font-black leading-none sm:text-6xl lg:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-4 border-[#38c3cc] pl-5 text-sm font-bold leading-7 text-white/85">
              Timely releases, public updates, and media-ready stories organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-[#b9dce4] bg-white text-[#07131c]">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[#006d86]' : 'hover:text-[#006d86]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#006d86]' : 'whitespace-nowrap hover:text-[#006d86]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] bg-white shadow-[0_16px_40px_rgba(8,38,59,.12)] lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-[#c8e2e8] lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[#38c3cc] px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-black">{getCategory(lead, label)}</span>
            ...
            <div className="grid">
              <div className="border-b border-[#c8e2e8] bg-[#38c3cc] p-6 text-black">
              ...
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-[#c8e2e8] bg-white last:border-b-0">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover grayscale transition group-hover:grayscale-0" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#006d86]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] bg-[#dff6fb] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-[#647b86] pb-4">
            <h2 className="text-4xl font-black sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex overflow-hidden rounded-full border-2 border-[#006d86] bg-white">
              ...
              <button className="h-11 bg-[#006d86] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-lg bg-white shadow-[0_12px_28px_rgba(8,38,59,.12)] transition hover:-translate-y-1">
                  <div className="aspect-[16/10] overflow-hidden bg-[#08263b]">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.18em] text-[#006d86]">
            ...
            <div className="rounded-lg border border-dashed border-[#006d86] bg-white p-12 text-center">
          ...
          <div className="mt-10 flex items-center justify-center gap-3">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border-2 border-[#006d86] bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            <span className="rounded-full bg-[#38c3cc] px-5 py-3 text-xs font-black uppercase text-black">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border-2 border-[#006d86] bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
